...
- A new `/v3/ws` endpoint alias (same handler as v2) with v3 defaulting when the client omits a version.
- Targeted peer-to-peer signaling via opaque payload relay (`Offer`, `Answer`, `IceCandidate`) between peers in the same room.
...
- New protocol/config guardrails:
  ...
  - Explicit error codes for unsupported transport, cross-room signaling, target-not-found, and signal rate limits.
