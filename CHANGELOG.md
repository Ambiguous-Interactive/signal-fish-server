...
- Add server-initiated RFC 6455 liveness probes (P10.E4). By default each
  ...
  change, map stalled Ping socket writes to `4003 activity_timeout`, reject
  guessed matching Pongs observed before the socket write begins, accept matching
  Pongs through the exact deadline, preserve the first matching reply against
