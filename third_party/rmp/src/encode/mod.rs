...
macro_rules! write_byteorder_utils {
    ($($name:ident => $method:ident, $tp:ident),* $(,)?) => {
        $(
            #[inline]
            #[doc(hidden)]
            fn $name(&mut self, val: $tp) -> Result<(), DataWriteError<Self::Error>>
            where
                Self: Sized,
            {
                const SIZE: usize = core::mem::size_of::<$tp>();
                let mut buf: [u8; SIZE] = [0u8; SIZE];
                <byteorder::BigEndian as byteorder::ByteOrder>::$method(&mut buf, val);
...
pub trait RmpWrite: sealed::Sealed {
    ...
    write_byteorder_utils!(
        write_data_u16 => write_u16, u16,
        write_data_u32 => write_u32, u32,
        write_data_u64 => write_u64, u64,
        write_data_i16 => write_i16, i16,
        write_data_i32 => write_i32, i32,
        write_data_i64 => write_i64, i64,
        write_data_f32 => write_f32, f32,
        write_data_f64 => write_f64, f64
    );
}
