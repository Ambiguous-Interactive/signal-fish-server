...
macro_rules! read_byteorder_utils {
    ($($name:ident => $method:ident, $tp:ident),* $(,)?) => {
        $(
            #[inline]
            #[doc(hidden)]
            fn $name(&mut self) -> Result<$tp, ValueReadError<Self::Error>>
            where
                Self: Sized,
            {
                ...
                Ok(<byteorder::BigEndian as byteorder::ByteOrder>::$method(&mut buf))
            }
        )*
    };
}
...
pub trait RmpRead: sealed::Sealed {
    ...
    read_byteorder_utils!(
        read_data_u16 => read_u16, u16,
        read_data_u32 => read_u32, u32,
        read_data_u64 => read_u64, u64,
        read_data_i16 => read_i16, i16,
        read_data_i32 => read_i32, i32,
        read_data_i64 => read_i64, i64,
        read_data_f32 => read_f32, f32,
        read_data_f64 => read_f64, f64
    );
}
...
macro_rules! wrap_data_funcs_for_compatibility {
    ($($fn_name:ident => $tp:ident),* $(,)?) => {
        $(
            #[cfg(feature = "std")]
            #[doc(hidden)]
            #[deprecated(note = "internal function. rmpv & rmp-serde need to switch to RmpRead")]
            pub fn $fn_name<R: std::io::Read>(
                buf: &mut R,
            ) -> Result<$tp, ValueReadError> {
                buf.$fn_name()
            }
        )*
    };
