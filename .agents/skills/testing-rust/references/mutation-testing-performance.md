...
**Applies to**: When mutation-testing CI is slow or timing out — `cargo-mutants` shards
...
- General application hot-path optimization (see
  [Rust Performance Optimization](../../rust-development/references/rust-performance-optimization.md))
- Writing the CI configuration tests themselves (see
  [CI Configuration Validation Tests](../../ci-troubleshooting/references/github-actions-config-tests.md))
- Generic workflow caching / action pinning (see
  [GitHub Actions Caching & Action Versioning](../../ci-troubleshooting/references/github-actions-caching.md))
...
3. **`mold` linker.** `scripts/run-mutants.sh` sets the linker via the per-target
   ...
   cross-linking stays simple — see `.agents/skills/documentation-quality/references/project-docs-and-ci-pitfalls.md`).
...
- **Per-shard timeout must stay within `[8, 15]` minutes.**
  - The **floor (8 min)** keeps headroom for a cold-cache miss or a loaded
    ...
    [Zero-Flakiness Policy](project-testing.md#zero-flakiness-policy-zero-tolerance).
...
- [Rust Performance Optimization](../../rust-development/references/rust-performance-optimization.md) — Profiles, linker, and build-time levers
- [CI Configuration Validation Tests](../../ci-troubleshooting/references/github-actions-config-tests.md) — How the guard-map tests are written
- [GitHub Actions Caching & Action Versioning](../../ci-troubleshooting/references/github-actions-caching.md) — Shared `rust-cache` keys, action pinning
