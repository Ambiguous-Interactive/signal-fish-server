...
See [Documentation Standards](documentation-standards.md) for full standards.
...
[Config and Wire-Format Drift](../../repository-maintenance/references/config-wire-format-drift.md)
...
- **`.lychee.toml` `exclude` patterns are regex, not globs** -- escape `.` as `\\.`,
  ...
  [CI/CD Troubleshooting Link Patterns](../../ci-troubleshooting/references/ci-cd-troubleshooting-links.md).
...
- **Avoid accidental setext headings in skills** -- keep a blank line between
  `**Applies to**: ...` and a following `---` separator, or markdownlint will treat
