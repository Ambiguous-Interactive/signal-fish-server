...
function Get-IndexBlobTexts {
    ...
    foreach ($rawLine in $sizeCheck.Stdout -split "`n") {
        ...
        if ($totalByteCount -gt $script:MaxBatchedBlobBytes) {
            throw "Staged blob batch is $totalByteCount bytes, above the $script:MaxBatchedBlobBytes byte pre-commit limit. Split oversized Agent Skill references."
        }
    }
...
function Get-DocVersionResidualProblem {
    ...
    switch ($Path) {
        ...
        "AGENTS.md" {
            ...
            return "AGENTS.md is missing the required line '$expectedLine' and cannot be auto-created. Add it under the project identity section."
        }
        default {
            return $null
        }
