#!/usr/bin/env bash
# Agent Skill File Size Enforcer
#
# Validates that all files in the .agents/skills/ directory are within the maximum
# allowed line count. Large files must be split into focused sub-files.
#
# The 300-line reference limit keeps progressively loaded material focused.
#
# Usage:
#   ./scripts/check-agent-skill-files.sh
#   ./scripts/check-agent-skill-files.sh --files .agents/skills/foo/SKILL.md
#
# Exit codes:
...
print_size_diagnostics() {
    ...
    echo ""
    echo "Largest checked Agent Skill files:"
    printf '%s\n' "${FILE_SIZE_RECORDS[@]}" \
...
echo -e "${BLUE}Agent Skill File Size Checker${NC}"
...
if [ "${1:-}" = "--files" ]; then
    ...
    if [ ! -d "$SKILLS_DIR" ]; then
        echo -e "${YELLOW}[WARN]${NC} Directory '$SKILLS_DIR' not found — skipping check"
        exit 0
    fi
...
if [ "$FILE_ARGS_MODE" -eq 1 ]; then
    ...
    info "Scanning files in $SKILLS_DIR/ for size violations..."
fi
echo ""
...
for file in "${FILES_TO_CHECK[@]}"; do
    ...
    case "$file" in
        .agents/skills/index.md|./.agents/skills/index.md) continue ;;
    esac
    ...
    if [ "$LINE_COUNT" -gt "$MAX_LINES" ]; then
        ...
        if [ "${GITHUB_ACTIONS:-}" = "true" ]; then
            echo "::error file=$file::Agent Skill file exceeds ${MAX_LINES}-line limit ($LINE_COUNT lines)"
        fi
        echo "       Split into focused sub-files of ≤${MAX_LINES} lines."
        echo "       See .agents/skills/repository-maintenance/references/manage-skills.md for guidance."
        ...
        if [ "$HEADROOM" -eq 0 ] && [ "$LINE_COUNT" -gt 0 ]; then
            ...
            if [ "${GITHUB_ACTIONS:-}" = "true" ]; then
                echo "::warning file=$file::Agent Skill file is at the ${MAX_LINES}-line limit"
            fi
            ...
            if [ "${GITHUB_ACTIONS:-}" = "true" ]; then
                echo "::warning file=$file::Agent Skill file is close to limit (${LINE_COUNT}/${MAX_LINES} lines)"
            fi
        fi
    fi
done
...
if [ "$FILE_ARGS_MODE" -eq 1 ]; then
    ...
    info "Checked $CHECKED file(s) in $SKILLS_DIR/"
fi
...
if [ "$VIOLATIONS" -gt 0 ]; then
    print_size_diagnostics
    error "Agent Skill file size check found $VIOLATIONS file(s) exceeding the ${MAX_LINES}-line limit"
    echo ""
    echo "Each Agent Skill Markdown file must stay within ${MAX_LINES} lines to:"
    echo "  - Stay focused on one topic"
    echo "  - Fit comfortably within agent context windows"
    echo "  - Remain scannable and maintainable"
    echo ""
    echo "How to fix:"
    echo "  1. Identify logically distinct sections in the oversized file"
    echo "  2. Keep task routing in SKILL.md and extract details into references/"
    echo "  3. Link every extracted reference directly from SKILL.md"
    echo "  4. Run ./scripts/generate-skills-index.sh to refresh the index"
    exit 1
elif [ "$WARNINGS" -gt 0 ]; then
    print_size_diagnostics
    echo -e "${YELLOW}[WARN]${NC} Agent Skill file size check passed with $WARNINGS warning(s)"
    echo "Files at or near the limit should be kept trim to prevent future violations."
    exit 0
else
    success "All $CHECKED Agent Skill Markdown file(s) are within the ${MAX_LINES}-line limit."
    exit 0
fi
