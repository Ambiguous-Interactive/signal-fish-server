...
#   Mutation testing rebuilds the crate once per mutant (~199 mutants). Naively,
...
#   bash scripts/run-mutants.sh --shard 0/16 --print-cmd   # print, do not run
#
# Env:
...
if [ "$MODE" = "warm" ]; then
    ...
    if ! printf '%s' "$SHARD" | grep -Eq '^[0-9]+/[0-9]+$'; then
        echo "ERROR: --shard must be <k>/<N> (e.g. 0/16), got: '${SHARD}'" >&2
        exit 2
    fi
