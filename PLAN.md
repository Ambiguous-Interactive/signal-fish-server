...
> changelog gate.
> P84 closes #344 by deriving token-binding identity from rustls's authenticated
> leaf certificate instead of request headers. The same session consolidates
> unused-dependency analysis behind one checkout, nightly toolchain, and Rust
> cache while preserving both historical check names until #345's external
> ruleset audit is possible.
> P85 closed #347 by adding server freshness, one cross-format proof sequence,
> portable JSON and binary proof envelopes, and atomic certificate identity
> checks for reconnect credentials. It also makes required CI checks fail closed
> behind the optional quick gate and replaces three measured source-build tool
> installs with pinned release binaries. P86 closes #345 after the effective
...
> P96 closes #364 by running the complete asynchronous rate-limit unit-test
> group against Tokio's paused test clock. This preserves explicit virtual-time
> reset tests while preventing a slow interpreter or contended host from expiring the
> 100-millisecond test fixture during one logical assertion sequence; production
> time and limiter behavior are unchanged.
...
- [x] Configure `windows-latest` and `macos-latest` to run that exact selector
  ...
  The full eight-scenario topology/fault matrix remains the Linux control.
...
full Rust, formal, Z3, policy, documentation, and hosted-CI gates pass. At P77
acceptance, P53/P56 remained evidence-only at 6/20 per OS and 6/20 respectively.
...
### P83 — Fail-closed TLS artifacts + deterministic interop teardown (Size M) — ✅ DONE

Session 124 closes two independent failures found while auditing the first
post-P82 `main` run. A binary built without the optional `tls` feature accepted
`tls.enabled=true` and then silently served plaintext; official container and
release builds omitted that feature. Separately, exact native/browser WebRTC
matrices released siblings after a fixed 250 ms local-success linger, so
teardown could erase an outstanding SCTP delivery obligation. The hosted
browser failure then asserted only the pre-`PlayerLeft` window even though SCTP
and WebSocket lifecycle events have no cross-transport ordering.

- [x] Reject TLS-enabled configuration when the binary lacks compiled TLS
  support; make token binding and TURN warning predicates use effective TLS.
- [x] Compile `--features tls` into official Docker and release artifacts and
  pin both build paths with CI configuration tests.
- [x] Remove untrusted request-header fingerprint capture, reject the reserved
  verified-fingerprint binding mode, and track real rustls peer-certificate
  extraction in issue #344.
- [x] Add browser parity for the native `--success-release-file` contract and
  prove the held process survives beyond its ordinary exit linger.
- [x] Gate exact native, browser, and positive TURN exchange matrices until
  every client emits `success_criteria_met`, then release them together.
- [x] Latch a connected peer's logical exchange debt across later departure
  and transport teardown in both reference clients, so ordinary non-harness
  runs cannot erase an incomplete reliable or unreliable direction.
- [x] Assert data-channel ledgers over complete drained logs rather than a
  WebSocket lifecycle window.
- [x] Preserve the browser's post-release linger when a held success barrier
  opens after the soft run deadline.
- [x] Record the measured but externally blocked unused-dependency workflow
  consolidation in issue #345 instead of risking deletion of a required check.

**Acceptance:** no-feature `--validate-config` fails against an otherwise
path-valid TLS configuration; official build commands include TLS; spoofable
fingerprint headers never reach token binding; the formerly failing
`browser_pair_mesh_n3` cell passes ten consecutive runs and the full browser
matrix passes without retry; mandatory local gates and hosted PR CI are green.

---

### P84 — Verified mTLS token binding + check-safe unused-deps consolidation (Size M) — ✅ DONE

Session 125 completes the fail-closed follow-up from P83. The built-in TLS
listener now reads the authenticated leaf certificate only after rustls has
verified the handshake, hashes its exact DER bytes with SHA-256, and installs
that identity on the per-connection request service. Direct client-supplied
fingerprint and forwarding headers remain inert.

- [x] Preserve the raw-socket `TCP_NODELAY` acceptor and wrap the completed TLS
  handshake with verified certificate metadata before Axum handles requests.
- [x] Define the wire fingerprint as 64 lowercase hexadecimal SHA-256
  characters over the leaf certificate's DER bytes.
- [x] Accept fingerprint binding only with token binding enabled, effective
  built-in TLS, mandatory token binding, and optional or required TLS client
  authentication; reject a subprotocol opt-out even when a library embedding
  bypasses process-level configuration validation.
- [x] Reject an optional-mTLS WebSocket without a certificate when fingerprint
  binding is required; never promote a candidate request header into identity.
- [x] Prove exact-cert success, conflicting-header resistance, old-proof
  rejection after certificate rotation, new-cert success, and unchanged
  non-fingerprint token binding through the real production binary/listener.
- [x] Keep the production TLS dependency graph on the explicit ring provider;
  direct `tokio-rustls` access disables default features so AWS-LC does not
  return to the ARMv7 artifact graph.
- [x] Run cargo-machete and cargo-udeps behind one checkout, nightly toolchain,
  and Rust cache while retaining `Check Unused Features` as a fail-closed,
  setup-free compatibility status until #345's administrator-only
  required-check audit can remove it.
- [x] Reject proof-less binary frames in fingerprint-bound mode and track the
  broader reconnect-credential/binary-envelope follow-up in #347 without
  overstating this phase's security boundary.
- [x] Record the complete mandatory local suite, adversarial zero-finding
  closure, publication, and green hosted PR checks.

**Acceptance:** the real listener binds a proof only to the certificate rustls
authenticated; missing, spoofed, conflicting, and rotated identities fail
closed; ordinary TLS/token binding remains compatible; the consolidated
workflow executes both analyzers with one shared setup and preserves both
historical status names; mandatory local gates and hosted PR CI are green.

**Completed:** PR #348 merged at exact reviewed head
`8a4bf85925de4fcb8808e00f5ae4239c46a98d3d`. The complete mandatory local
suite and three independent adversarial reviews passed. The first hosted head
exposed two panic-policy lint failures in the new hex encoder; the follow-up
made nibble conversion fallible and fail-closed, and the exact hosted command
then passed locally. On the final head, all 20 indexed PR workflow runs settled
without a failure (18 succeeded and two inapplicable Dependabot auto-merge runs
skipped), including both consolidated unused-dependency status names, strict
panic policy, ASan, Miri, fuzzing, formal verification, every interop lane, and
Linux/macOS/Windows nextest. The current `main` SHA also has all 17 applicable
push workflows green. GitHub reports no review thread or requested change;
Copilot's only comments report quota exhaustion, while the independent reviews
closed with zero remaining release blockers.

---

### P85 — Replay-resistant token binding + fail-closed/lean CI (Size M) — ✅ DONE

Session 126 implements the security boundary deliberately carried from P84. Token
binding v1 used only a client-chosen WebSocket key, had no proof sequence or
portable numeric canonicalization, accepted proof-less binary traffic outside
fingerprint mode, and issued reconnect bearer credentials without retaining the
verified certificate identity. The same session closes a CI status-graph gap
and removes measured source-build overhead without changing analyzer policy.

- [x] Send a 256-bit server-fresh challenge as the first application message on
  every token-bound connection and derive a 256-bit key with HKDF-SHA-256 over
  both handshake inputs.
- [x] Require a versioned proof and one exact monotonic sequence across all JSON
  and binary client frames; reject cross-connection replay, gaps, duplicates,
  invalid signatures, and proof-less binary frames before application logic.
- [x] Canonicalize JSON with RFC 8785 and pin cross-language numeric and complete
  HKDF/HMAC golden vectors.
- [x] Carry binary game data in a signed, named-field MessagePack outer envelope
  while leaving its inner legacy payload bytes unchanged.
- [x] Persist the rustls-verified certificate identity from join-time token
  pre-issuance through disconnect, reconnect reassignment, and rollback; preserve
  the original identity on duplicate same-room teardown.
- [x] Compare reconnect identity under the same replay-state write lock before
  reserving a claim, in constant time for matching-length values; a missing or
  rotated identity cannot consume the valid credential.
- [x] Document the exact wire contract and certificate-rotation policy: an
  outstanding A-bound token requires A, while B obtains a new credential through
  the normal join flow.
- [x] Make every required CI job behind `Quick Check` run after prerequisite
  failure, but not workflow cancellation, and fail closed before setup when the
  optional prerequisite does not succeed.
- [x] Replace measured cargo-machete, cargo-udeps, and Taplo source builds with
  exact-version, checksum-verified release binaries using the repository's
  existing pinned installer action. Analyzer commands and failure policy remain
  unchanged; #345's required-check alias audit remains administrator work.
- [x] Record the complete mandatory local suite, adversarial zero-finding
  closure, publication, and green hosted PR checks.

**Acceptance:** captured v2 proofs fail on a fresh socket; JSON and binary share
one replay frontier and match the documented portable vectors; certificate B
cannot claim or consume A's reconnect token while A can still reconnect; default
non-token TLS, JSON, binary, and reconnect paths remain unchanged. A failed
quick gate cannot leave required downstream statuses skipped/successful, the
unused-dependency cold path no longer compiles its analyzers, and all mandatory
local and hosted gates pass.

**Completed:** PR #349 is fully green at exact implementation head
...
auto-merge run skipped). One Rustdoc job initially failed during checkout
because its runner had no usable CA file; the same job passed on its targeted
rerun without a source change. The first hosted heads also exposed stale
standalone lockfiles, an invalid JSON documentation fragment, a reserved
subprotocol/version mismatch reported by Bugbot, and an unchecked sequence
increment rejected by Panic Policy; each was fixed at its source and covered by
the relevant regression gate. The sole review thread is resolved, and three
independent adversarial reviews closed with zero remaining findings. The
prebuilt analyzer install took 0.93 seconds and the complete hosted unused-
dependency job took about 110 seconds, down from the audited 526-second cold
main run.
...
but has no required status contexts or checks. Its only active repository
ruleset applies deletion, non-fast-forward, and Copilot-review rules to the
default branch and contains no required-status-check rule. The historical
`Check Unused Features` alias therefore has no merge-policy consumer.

- [x] Audit both classic branch protection's effective required-status list and
  every active repository ruleset through the connected GitHub app.
- [x] Remove the setup-free `Check Unused Features` compatibility job while
  retaining the gating `Check Unused Dependencies` analyzer job.
- [x] Keep cargo-machete and cargo-udeps behind exactly one checkout, nightly
  toolchain, Rust cache, and checksum-verified binary install.
- [x] Preserve cargo-machete as gating and cargo-udeps as informational while
  still running udeps after a machete failure unless the workflow is cancelled.
- [x] Strengthen the CI configuration regression test to require exactly one
  hosted job and reject reintroduction of the audited obsolete alias.
- [x] Remove documentation validation's no-op inline-code placeholder job; it
  checked out the repository only to print that validation was skipped.
...
hosted gates pass. The removal is expected to save one runner allocation per
push or PR synchronization (the #345 baseline measured about 21.6 seconds / 0.36
raw runner-minutes of avoidable setup per trigger). Removing the no-op docs job
...
interpreter crossed the unit fixture's 100-millisecond window between logically
adjacent assertions, reset the entry, and observed zero consumed signals. The
production limiter's default window is one minute, but that production value was
not involved in the failure.

- [x] Preserve the below-limit proof that a signal-availability preflight leaves
  the consumed count at zero and the above-limit proof that rejection leaves the
  consumed count at two.
- [x] Sweep every asynchronous `RoomRateLimiter` unit test that shares the short
  window fixture onto Tokio's paused test clock rather than repairing only the
  observed test.
- [x] Retain explicit virtual-time advances for window-reset and cleanup behavior,
