...
>
> Status: ACTIVE. **All in-repo work through P52, P54–P55, and P57–P84 is complete;
> P85 is implemented locally and awaiting hosted validation; P53 is collecting
...
> allocations per OS; P56 has six of 20 eligible scheduled H14 attempts.** The M1
> server core, the M2
...
> formal-verification work beyond the bounded exposure theorem, #318's
> remaining cross-platform developer-hook latency evidence, and #345's
> administrator-level required-check audit before the legacy unused-features
> status alias can be removed. #347's replay-resistant token binding and
...
> P85 is intended to close #347 by adding server freshness, one cross-format proof sequence,
...
> installs with pinned release binaries while leaving #345's administrator-only
> status-alias audit open.
...
| P85 | Replay-resistant token binding + fail-closed/lean CI (#347, #345) | M | P84 | Security / CI | 🟡 Validating (s126) |
...
remain evidence-only at 6/20 per OS and 6/20 respectively.
...
### P85 — Replay-resistant token binding + fail-closed/lean CI (Size M) — 🟡 VALIDATING
...
- [ ] Record the complete mandatory local suite, adversarial zero-finding
  closure, publication, and green hosted PR checks.
...
hosted evidence gates; P85 awaits hosted validation of its implemented in-repo
security and CI closure;
