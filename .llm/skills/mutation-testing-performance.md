...
3. **Per-mutant relink.** Every mutant relinks the crate; a slow linker multiplies
   that across 356 mutants × N shards.
...
7. **Shard count sized for serial execution.** `--in-place` runs each shard's
   ...
   rounding up gives a 29s/mutant budget. Resharded to **36**: 356 mutants ÷ 36
...
```text
{ mutant-count (356), shard-count N, per-shard timeout, per-mutant budget }
```
...
`ceil(356/36) × 29s = 10 × 29s = 290s`, below the 5-minute target. The
