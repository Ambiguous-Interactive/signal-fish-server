# Skill: CI/CD Troubleshooting Example - Python Cache on Rust Project

<!--
  trigger: ci example python cache, rust ci pip cache mismatch, cache deserialization failure
  | Example incident: Python cache configured on a Rust CI workflow
  | Infrastructure
-->

**Trigger**: When CI on a Rust project fails with pip/cache-related errors due to ecosystem cache mismatch.
