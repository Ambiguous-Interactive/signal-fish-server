...
7. **Shard count sized for serial execution.** `--in-place` runs each shard's
   ...
   rounding up gives a 29s/mutant budget. The current **40** shards cover 392
...
```text
{ mutant-count (392), shard-count N, per-shard timeout, per-mutant budget }
```
...
`ceil(392/40) × 29s = 10 × 29s = 290s`, below the 5-minute target. The
