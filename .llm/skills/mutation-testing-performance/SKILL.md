...
7. **Shard count sized for serial execution.** `--in-place` runs each shard's
   ...
   rounding up gives a 29s/mutant budget. The current **39** shards cover 390
...
```text
{ mutant-count (390), shard-count N, per-shard timeout, per-mutant budget }
```
...
29s/mutant budget. Using 39 shards caps the modeled largest shard at
`ceil(390/39) × 29s = 10 × 29s = 290s`, below the 5-minute target. The
