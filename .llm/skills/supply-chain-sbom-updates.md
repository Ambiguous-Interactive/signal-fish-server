# Skill: Supply Chain Security — SBOMs, Updates, and CI Integration

<!--
  trigger: sbom, dependabot, renovate, cargo-sbom, supply-chain-ci, cargo-lock-version, cargo-deny-action
  | SBOM generation, dependency update policy, CI pipeline integration, action version compatibility
  | Security
-->

**Trigger**: When generating SBOMs, configuring automated dependency updates, setting up the supply chain
CI pipeline, or checking CI action compatibility with the Cargo.lock version.
