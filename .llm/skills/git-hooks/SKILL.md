...
name: git-hooks
description: >-
  Install, implement, test, and optimize cross-platform Git hooks for this repository. Use when
  changing pre-commit or pre-push behavior, hook installation, staged-file checks, performance,
  or hook diagnostics.
---
...
- Read [Hook Checks](references/checks.md) when implementing, testing, profiling, or debugging a
  staged-file check.
- Read [Fast Pre-Commit Sample](references/pre-commit-fast.sh) when designing hook control flow.
- Read [Performance Patterns](references/performance-patterns.sh) when optimizing repeated Git or
  process operations.
- Read [CI Hook Validation Tests](references/ci-hook-validation-tests.rs) when adding regression
  coverage for hook behavior.
- Read [Debugging Snippets](references/debugging-snippets.sh) when collecting hook diagnostics.
...
- [Git Hooks Checks](references/checks.md) — Pre-commit hook design, checks, testing, debugging
- [GitHub Actions Workflow Config](../github-actions-workflow-config/SKILL.md) — CI/CD workflow patterns
- [Shell Scripting Patterns](../shell-scripting-patterns/SKILL.md) — Shell scripting best practices
