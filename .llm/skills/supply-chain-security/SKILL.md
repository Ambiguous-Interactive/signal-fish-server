...
description: >-
  Audit and harden the Rust and GitHub Actions supply chain, including vulnerabilities,
  licenses, lockfiles, SBOMs, dependency automation, and reproducible builds. Use for security
  audits, dependency incidents, or supply-chain CI changes.
---

# Supply Chain Security — Audit Tools and Policy
...
- SBOM generation and update automation (see [Supply Chain Sbom Updates](references/sbom-and-updates.md))
- Application-level security (see [Web Service Security Auth](../web-service-security/SKILL.md))
...
Docker multi-stage builds must also use `--locked`. See [Container Docker](../containers/SKILL.md)
...
[Supply Chain Example Rustls Pemfile To Rustls Pki Types](references/example-rustls-pemfile-migration.md).
...
4. **Add ban if warranted** — if the crate should never be used again,
   ...
   [Dependency Management Cargo](../dependency-management/SKILL.md).
...
- [Supply Chain Sbom Updates](references/sbom-and-updates.md) — SBOM generation, update policy,
...
- [Web Service Security Hardening](../web-service-security/references/hardening-and-operations.md) —
