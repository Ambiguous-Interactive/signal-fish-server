---
name: rust-refactoring-guide
description: >-
  Apply project guidance for Rust refactoring. Use when restructuring, extracting,
  splitting, or modernizing existing Rust code.
---
...
- Writing brand new code from scratch (see [Rust Idioms And Patterns](../rust-idioms-and-patterns/SKILL.md))
- Performance-focused changes (see [Rust Performance Optimization](../rust-performance-optimization/SKILL.md))
...
See [Error Handling Guide](../error-handling-guide/SKILL.md) for the full unwrap hierarchy and `?` propagation patterns.
...
- **Sync → Async:** Add `async`, replace blocking I/O with `tokio::fs`, add `.await`,
  ...
  See [Async Rust Best Practices](../async-rust-best-practices/SKILL.md).
- **Reduce clone():** Pass `&T`, use `Arc<T>` for shared ownership, `Bytes` for network data,
  `Cow<str>` for conditional ownership.
  See [Rust Performance Optimization](../rust-performance-optimization/SKILL.md).
...
See [Clippy And Linting](../clippy-and-linting/SKILL.md) for full clippy configuration.
...
- [Rust Idioms And Patterns](../rust-idioms-and-patterns/SKILL.md) — Target patterns for refactoring
- [Clippy And Linting](../clippy-and-linting/SKILL.md) — Automated fixes with clippy
- [Error Handling Guide](../error-handling-guide/SKILL.md) — Refactoring unwrap chains
- [Testing Core Patterns](../testing/SKILL.md) — Tests must pass before and after refactoring
- [Code Review Checklist](../code-review-checklist/SKILL.md) — AI-driven code review with structured output
- [Solid Principles Enforcement](../solid-principles-enforcement/SKILL.md) — SOLID principle enforcement during refactoring
