...
advertises `["websocket"]`; it is omitted with the other v3-only fields on negotiated v2 connections. Negotiated v3
also carries `implementation_version`, the exact release of the running server implementation, so a client can pin
the deployment it tested against.
