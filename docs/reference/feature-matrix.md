...
| Token binding | both | `security.transport.token_binding.enabled` (+ `required`, `subprotocol`, `scheme`); `require_client_fingerprint` additionally requires `required=true`, built-in TLS, and `tls.client_auth=optional` or `require` | Negotiate the token-binding subprotocol and send signed JSON messages; fingerprint-bound connections reject unsigned binary frames and require the authenticated leaf certificate's lowercase hex SHA-256 digest in each proof |
| MessagePack game data | both | `protocol.enable_message_pack_game_data` (default `true`); unavailable when token binding requires a client-certificate fingerprint because its binary carrier has no proof envelope | `Authenticate` with `game_data_format` of `message_pack`; decode the binary `GameDataBinary` carrier |
