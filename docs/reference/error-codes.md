...
| `CONNECT_TOKEN_REQUIRED` | `Authenticate` carried no `connect_token`, but the deployment (or this application's registration) requires one. Obtain a token from the game's backend and retry on the same connection. Distinct from `CONNECT_TOKEN_INVALID` so clients can tell "obtain a token first" from "your token is bad". See [Application identification](../authentication.md#optional-tenant-connect-tokens). |
...
| `BANNED` | This player id is banned from the room by its authority player (`BanPlayer`) and cannot join it, as a player or spectator, or restore its seat through reconnection, while the ban lives. The ban is room-scoped and expires with the room; the authority can lift it with `UnbanPlayer` (a pending reconnection record is kept, so a mid-window unban lets the token work again). |
