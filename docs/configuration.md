...
| `SIGNAL_FISH__SECURITY__MAX_CONNECTION_INFO_BYTES` | `security.max_connection_info_bytes` | `8192` | Max serialized size in bytes of one `ProvideConnectionInfo` peer-metadata entry (must be > 0, ≤ `max_message_size`, and `(cap + per-member envelope overhead) × max_players_limit` must not exceed `max_outbound_message_size`) |
