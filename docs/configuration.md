...
| `SIGNAL_FISH__SECURITY__ALLOWED_APPS` | `security.allowed_apps` | `[]` | JSON array of public app-ID registrations and accounting limits (`max_rooms`, `max_players_per_room`, `rate_limit_per_minute`, and `max_relay_bytes` must be > 0 when set) |
