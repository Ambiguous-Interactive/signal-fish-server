...
| `SIGNAL_FISH__SECURITY__CONNECT_TOKEN` | `security.connect_token` | `null` | Optional tenant `connect_token` verification (issue #517): `{"public_key": "<base64 32-byte Ed25519 public key>", "public_key_path": "<file>"}`; set exactly one source — the loader folds the file into `public_key`, and a missing file is a startup error. The key is public material (the private key stays in the control plane). Reloads on SIGHUP. See `docs/authentication.md` |
