...
| `SIGNAL_FISH__SECURITY__ALLOWED_APPS` | `security.allowed_apps` | `[]` | JSON array of public app-ID registrations and accounting limits (`max_rooms`, `max_players_per_room`, `rate_limit_per_minute`, `max_relay_bytes`, and `require_connect_token`; the accounting limits must be > 0 when set) |
| `SIGNAL_FISH__SECURITY__APP_AUTH_PATH` | `security.app_auth_path` | `null` | Path to an external app-registry file (`{"apps": [...]}`, same entry shape as `allowed_apps`); entries are appended to the allowlist at startup and admission is fail-closed (a missing or invalid file is a startup error) |
| `SIGNAL_FISH__SECURITY__CONNECT_TOKEN` | `security.connect_token` | `null` | Optional tenant `connect_token` verification (issue #517): `{"public_key": "<base64 32-byte Ed25519 public key>", "public_key_path": "<file>", "required": false}`; set exactly one key source — the loader folds the file into `public_key`, and a missing file is a startup error. The key is public material (the private key stays in the control plane). `required: true` (issue #574) refuses token-less `Authenticate` handshakes with `CONNECT_TOKEN_REQUIRED`; per-app `require_connect_token` entries override per tenant. Reloads on SIGHUP. See `docs/authentication.md` |
...
| `SIGNAL_FISH__TURN__CREDENTIAL_TTL_SECS` | `turn.credential_ttl_secs` | `3600` | Lifetime in seconds of a minted TURN credential (max `86400` when enabled) |
...
zero together when the limiter next consults them after `time_window` has
elapsed since their window started (re-anchoring the window at that moment),
so a player can spend the full budget at the end of one window and again at
the start of the next (up to twice the configured count across a boundary).
This is distinct from the per-application handshake limiter
