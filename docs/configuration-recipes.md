...
at most 60000 (1 minute), and `max_sojourn_ms` must exceed it when
`enable_batching` is `true` (all enforced at startup). Keep `idle_timeout_secs` positive in production — it reclaims zombie
