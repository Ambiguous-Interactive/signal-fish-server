...
async fn finalized_join_counts_its_mixed_membership_once_per_publication() {
    ...
    // The incumbent drifts to relay-only after the sticky mesh+WebRTC plan was
    // stored: the publication proceeds over a mixed membership and must count
    // the incapable member exactly once (issue #421 observation seam).
    ...
    assert!(
        server
            ...
            .await,
        "a drifted incumbent must not degrade the finalized join publication"
    );
    ...
    assert_eq!(
        server
            ...
            .mixed_path_members_observed,
        observed_before + 1,
        "the finalized join publication counts its mixed membership exactly once"
    );
    assert!(matches!(
        recv(&mut fixture.joiner_rx).await.as_ref(),
        ServerMessage::SessionPlan(plan)
