...
///
/// Clients must use a fresh UUID for every live or recently retained operation
/// on one authenticated physical connection. The server never interprets the
/// value; it echoes it on the corresponding terminal response. A new physical
/// connection starts a new identifier scope, including reconnect attempts.
...
/// Legacy relay transport preference tokens.
///
/// These tokens remain on the wire for compatibility, but the server does not
/// use them to select a data path. All values currently behave identically and
/// game data retains its authenticated WebSocket relay floor.
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq, Default)]
#[serde(rename_all = "lowercase")]
pub enum RelayTransport {
    /// Reserved TCP preference token (currently ignored).
    Tcp,
    /// Reserved UDP preference token (currently ignored).
    Udp,
    /// Reserved WebSocket preference token (currently ignored).
    Websocket,
    /// Reserved automatic preference token (currently ignored).
    #[default]
    Auto,
}
...
pub enum ConnectionInfo {
    ...
    /// Client-supplied legacy relay metadata, stored and forwarded verbatim.
    ///
    /// The signaling server does not resolve or probe the endpoint,
    /// authenticate the token, bind `client_id` to the source connection, or
    /// select/open this path. A consuming peer or integration owns endpoint
    /// reachability, credential, and source-identity validation before use.
    #[serde(rename = "relay")]
    Relay {
        /// Claimed relay host; not resolved or probed by the signaling server.
        host: String,
        /// Claimed relay port; reachability is not validated.
        port: u16,
        /// Self-declared legacy transport preference.
        #[serde(default)]
        transport: RelayTransport,
        /// Self-declared allocation identifier.
        allocation_id: String,
        /// Opaque client-supplied credential; not authenticated by this server.
        token: String,
        /// Claimed external client identifier; not source-bound by this server.
        #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct PeerConnectionInfo {
    ...
    ///
    /// Informational only: it does not select, open, authenticate, or prove a
    /// physical transport.
...
/// Capability token for correlated room-membership operation envelopes.
