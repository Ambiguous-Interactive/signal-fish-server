...
fn run_checker_with_fixture_and_env(
    ...
    let temp_root = unique_temp_dir("agent-skill-size-check");
    let script_src = repo_root().join("scripts/check-agent-skill-files.sh");
    let script_dst = temp_root.path().join("scripts/check-agent-skill-files.sh");
    ...
    command.arg("scripts/check-agent-skill-files.sh");
    for arg in args {
        command.arg(arg);
    }
...
#[test]
fn test_repository_agent_skill_files_stay_within_size_policy() {
    let root = repo_root();
    let mut command = bash_command();
    let output = command
        .arg("scripts/check-agent-skill-files.sh")
        .current_dir(&root)
        .output()
        .unwrap_or_else(|e| {
            panic!(
                "Failed to run Agent Skill file size checker in {}: {e}",
                root.display()
            )
        });
    ...
    assert!(
        ...
        "repository .agents/skills files must stay within the 300-line policy so cargo test catches \
...
#[test]
fn test_agent_skill_file_size_checker_data_driven_cases() {
    let cases = vec![
        ScriptCase {
            name: "passes_when_all_files_within_limit",
            files: vec![fixture_file(".agents/skills/ok.md", make_lines(42))],
            args: vec![],
            expected_exit: 0,
            must_contain: vec![
                "[INFO] Checked 1 file(s) in .agents/skills/",
                "[OK] All 1 Agent Skill Markdown file(s) are within the 300-line limit.",
            ],
        },
        ScriptCase {
            name: "warns_when_file_hits_limit",
            files: vec![fixture_file(".agents/skills/near-limit.md", make_lines(300))],
            args: vec![],
            expected_exit: 0,
            must_contain: vec![
                "[WARN] .agents/skills/near-limit.md: 300 lines (at limit — next added line will fail)",
                "[WARN] Agent Skill file size check passed with 1 warning(s)",
            ],
        },
        ScriptCase {
            name: "fails_when_file_exceeds_limit",
            files: vec![fixture_file(".agents/skills/too-long.md", make_lines(301))],
            args: vec![],
            expected_exit: 1,
            must_contain: vec![
                "[ERROR] .agents/skills/too-long.md: 301 lines (max: 300 — exceeds by 1)",
                "[ERROR] Agent Skill file size check found 1 file(s) exceeding the 300-line limit",
            ],
        },
        ScriptCase {
            name: "files_mode_skips_generated_index",
            files: vec![
                fixture_file(".agents/skills/index.md", make_lines(800)),
                fixture_file("AGENTS.md", make_lines(5)),
            ],
            args: vec!["--files", ".agents/skills/index.md", "AGENTS.md"],
            expected_exit: 0,
            must_contain: vec![
        ...
        ScriptCase {
            ...
            files: vec![
                fixture_file(".agents/skills/index.md", make_lines(800)),
                fixture_file(".agents/skills/ok.md", make_lines(5)),
            ],
            args: vec![],
            expected_exit: 0,
            must_contain: vec![
                "[INFO] Scanning files in .agents/skills/ for size violations...",
                "[INFO] Checked 1 file(s) in .agents/skills/",
                "[OK] All 1 Agent Skill Markdown file(s) are within the 300-line limit.",
            ],
        },
        ScriptCase {
            ...
            files: vec![
                fixture_file(".agents/skills/index.md", make_lines(800)),
                fixture_file(".agents/skills/references/index.md", make_lines(301)),
            ],
            args: vec![],
            expected_exit: 1,
            must_contain: vec![
                "[INFO] Checked 1 file(s) in .agents/skills/",
                "[ERROR] .agents/skills/references/index.md: 301 lines (max: 300 — exceeds by 1)",
            ],
        },
        ScriptCase {
            name: "files_mode_warns_for_missing_paths",
            files: vec![fixture_file("AGENTS.md", make_lines(8))],
            args: vec!["--files", ".agents/skills/missing.md", "AGENTS.md"],
            expected_exit: 0,
            must_contain: vec![
                "[WARN] Skipping non-existent file: .agents/skills/missing.md",
        ...
        ScriptCase {
            ...
            files: vec![fixture_file(".agents/skills/space file.md", make_lines(12))],
            args: vec!["--files", ".agents/skills/space file.md"],
            expected_exit: 0,
...
fn test_warning_threshold_matches_documented_policy() {
    ...
    for (line_count, warning_fragment) in cases {
        ...
        if let Some(fragment) = warning_fragment {
            assert!(
                output.contains("[WARN] .agents/skills/threshold.md")
...
fn test_warning_diagnostics_consume_all_sorted_input_under_pipefail() {
    let script = fs::read_to_string(repo_root().join("scripts/check-agent-skill-files.sh"))
        .expect("failed to read Agent Skill size checker script");
    assert!(
    ...
    let files = (1..=16)
        .map(|index| {
            fixture_file(
                &format!(".agents/skills/near-limit-{index:03}.md"),
                make_lines(300),
            )
        })
        .collect::<Vec<_>>();
    ...
    assert!(
        output.contains("Largest checked Agent Skill files:")
            && output.contains("[WARN] Agent Skill file size check passed with 16 warning(s)"),
