...
const MUTATION_TOTAL_MUTANTS: u32 = 349;
...
fn test_mutation_workflow_uses_fast_linker_and_in_place() {
    ...
    if !mutants_block
        ...
        .flat_map(str::lines)
        .any(|line| {
            line.trim() == "run: bash scripts/run-mutants.sh --shard ${{ matrix.shard }}/36"
        })
    {
        violations.push(
            "mutation.yml mutants job must run mutation via the exact `bash \
             scripts/run-mutants.sh --shard ${{ matrix.shard }}/36` command, not an inline or \
