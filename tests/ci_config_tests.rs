...
const REQUIRED_WORKFLOW_FILES: &[(&str, &str)] = &[
    ...
    (
        "ci-safety.yml",
        "Gating advanced safety analysis (Miri and AddressSanitizer)",
    ),
    (
        "llm-file-sizes.yml",
    ...
    for &(dependency, version) in expected {
        let found = ignores.iter().any(|entry| {
            let dependency_matches = entry
                ...
                .and_then(Yaml::as_str)
                == Some(dependency);
            let Some(Yaml::Sequence(versions)) = entry.as_mapping_get("versions") else {
                return false;
            };
            dependency_matches
                && versions
                    .iter()
                    .any(|configured| configured.as_str() == Some(version))
        });
        assert!(
            found,
...
const SAFETY_JOB_CONTINUE_ON_ERROR: &[(&str, bool)] = &[("miri", false), ("asan", false)];
...
fn test_ci_safety_workflow_failure_policy_is_explicit() {
    // Both safety analyzers must propagate failures to the workflow.
...
fn test_ci_safety_keeps_full_miri_and_asan_coverage() {
    ...
    for required in [
        ...
        "--target x86_64-unknown-linux-gnu --lib --all-features --no-fail-fast",
    ] {
        assert!(
...
const NO_PANIC_SCANNER_REQUIRED_MARKERS: &[(&str, &str)] = &[
    ...
    (
        "join(\"native\").join(\"src\")",
        "scanner must include the standalone native production source root",
    ),
    (
        "verified_out_of_line_test_modules",
        "test files must be exempted only through cfg(test) module declarations",
    ),
];
...
fn test_check_no_panics_script_structure() {
    ...
    assert!(
        clippy_commands.iter().any(|command| {
            command_has_flag_value(command, "--manifest-path", "clients/native/Cargo.toml")
                && command_has_flag(command, "--lib")
                && command_has_flag(command, "--bins")
                && command_has_flag(command, "--all-features")
        }),
        "check-no-panics.sh must lint the standalone native package production targets"
    );
    for required_lint in ["clippy::string_slice", "clippy::arithmetic_side_effects"] {
        assert!(
            clippy_commands
                .iter()
                .all(|command| command.iter().any(|part| part == required_lint)),
            "every panic-policy Clippy command must enforce {required_lint}"
        );
    }
...
#[test]
#[cfg(unix)]
fn test_check_no_panics_fails_closed_without_cargo() {
    let root = repo_root();
    let output = std::process::Command::new("/bin/bash")
        .arg(root.join("scripts/check-no-panics.sh"))
        .arg("patterns")
        .env("PATH", "/bin")
        .current_dir(&root)
        .output()
        .expect("bash must execute the panic-policy script");

    assert!(
        !output.status.success(),
        "missing Cargo must fail the policy gate"
    );
    assert!(String::from_utf8_lossy(&output.stderr).contains("cargo not found"));
}
...
const MUTATION_TOTAL_MUTANTS: u32 = 384;
...
///
/// Accept both a compact flow sequence and Prettier's multiline flow layout so
/// the policy guard remains compatible with the repository's formatter.
fn parse_mutation_shard_matrix(workflow: &str) -> Option<Vec<u32>> {
    ...
    while let Some(line) = lines.next() {
        ...
        let shard_indent = line.len() - trimmed.len();

        let mut scalar = strip_yaml_inline_comment(rest).trim().to_string();
        if scalar.is_empty() {
            let next = lines.next()?;
            let next_trimmed = next.trim_start();
            let next_indent = next.len() - next_trimmed.len();
            if next_indent <= shard_indent {
                return None;
            }
            scalar.push_str(strip_yaml_inline_comment(next_trimmed).trim());
        }
        if !scalar.starts_with('[') {
            return None;
        }

        while !scalar.contains(']') {
            let next = lines.next()?;
            let next_trimmed = next.trim_start();
            let next_indent = next.len() - next_trimmed.len();
            if next_indent <= shard_indent {
        ...
        let end = scalar[1..].find(']').map(|offset| 1 + offset)?;
        if !scalar[end + 1..].trim().is_empty() {
            return None;
        }
        let inner = &scalar[1..end];
        let mut members: Vec<&str> = inner.split(',').map(str::trim).collect();
...
#[test]
fn mutation_shard_matrix_parser_accepts_formatted_flow_sequences() {
    let cases = [
        ("      shard: [0, 1, 2]", vec![0, 1, 2]),
        (
            "      shard:\n        [\n          0,\n          1,\n          2,\n        ]",
            vec![0, 1, 2],
        ),
    ...
    for (workflow, expected) in cases {
        assert_eq!(parse_mutation_shard_matrix(workflow), Some(expected));
    }
}

#[test]
fn mutation_shard_matrix_parser_rejects_malformed_values() {
    let cases = [
        "      shard:\n        unrelated: [0, 1, 2]",
        "      shard: [0, 1, 2, bad]",
        "      shard: [0,,1,2]",
    ...
    for workflow in cases {
        assert_eq!(parse_mutation_shard_matrix(workflow), None);
    }
}
...
fn test_mutation_workflow_uses_fast_linker_and_in_place() {
    ...
    if !mutants_block
        ...
        .flat_map(str::lines)
        .any(|line| {
            line.trim() == "run: bash scripts/run-mutants.sh --shard ${{ matrix.shard }}/40"
        })
    {
        violations.push(
            "mutation.yml mutants job must run mutation via the exact `bash \
             scripts/run-mutants.sh --shard ${{ matrix.shard }}/40` command, not an inline or \
    ...
    assert!(
        ...
        "Mutation-testing fast-linker / in-place policy violations:\n\n{}\n\n\
         ...
         Verify: bash scripts/run-mutants.sh --shard 0/40 --print-cmd",
        violations.join("\n")
    );
}
...
fn test_mutation_shard_matrix_is_complete_contiguous_partition() {
    ...
    let workflow = read_mutation_workflow();
    let shards = parse_mutation_shard_matrix(&workflow).unwrap_or_default();
    ...
    for line in workflow.lines() {
        // Job name: "... shard ${{ matrix.shard }}/<N>"
        // Run step: "... --shard ${{ matrix.shard }}/<N>"
