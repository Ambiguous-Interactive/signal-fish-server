...
const MUTATION_TOTAL_MUTANTS: u32 = 390;
...
fn test_mutation_workflow_uses_fast_linker_and_in_place() {
    ...
    if !mutants_block
        ...
        .flat_map(str::lines)
        .any(|line| {
            line.trim() == "run: bash scripts/run-mutants.sh --shard ${{ matrix.shard }}/39"
        })
    {
        violations.push(
            "mutation.yml mutants job must run mutation via the exact `bash \
             scripts/run-mutants.sh --shard ${{ matrix.shard }}/39` command, not an inline or \
    ...
    assert!(
        ...
        "Mutation-testing fast-linker / in-place policy violations:\n\n{}\n\n\
         ...
         Verify: bash scripts/run-mutants.sh --shard 0/39 --print-cmd",
        violations.join("\n")
    );
}
