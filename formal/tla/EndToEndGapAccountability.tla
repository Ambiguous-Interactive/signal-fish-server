...
(*           upsert, not a replay of deltas — so a PlayerLeft that was       *)
(*           dropped from the wiped buffer (or truncated from the replay     *)
(*           ring) cannot strand a phantom member. `NoSnapshotReconcileBug`  *)
(*           applies only the (possibly incomplete) delta replay and TLC     *)
(*           catches the stranded member at quiescence.                      *)
...
(* checked configuration instantiates a single one.                          *)
(*                                                                          *)
...
(* house convention for an eventually-property under TLC.                       *)
